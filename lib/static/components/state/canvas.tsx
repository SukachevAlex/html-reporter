...
export default class CanvasComponent extends React.Component<IMyCanvasComponent> {
    ...
    updateCanvas() {
        ...
        imageObj.onload = function() {
            drawImage(this);
            let imgd = ctx && ctx.getImageData(0, 0, canvWidth, canvHeight);
            let pix = imgd && imgd.data;
            let pixMatr = pix && matrixArray(pix, canvHeight, canvWidth);
            ...
            for (let i = 0; i < canvHeight; i++) {
                for (let j = 0; j < canvWidth; j++) {
                    if (pixMatr[i][j][0] == 255 && pixMatr[i][j][1] == 0 && pixMatr[i][j][2] == 255) {
                        ...
                        for(let k = 0; k < boundBoxArr.length; k++) {
                            let row = boundBoxArr[k];
